...
int main(int argc, char *argv[])
{
    ...
    for (i=1; i<argc; i++)
    {
        if (ReadParam(argv[i]) < 0)
        {
            fprintf(stderr, "Failed to read parameter: %s\n", argv[i]);
            exit(-1);
        }
        paramFlag = 1;
    }
    ...
    if ((DeviceSoc = init_socket(Param.device)) < 0)
    {
    ...
    {
        fprintf(stderr, "Failed to check GARP\n");
        exit(-1);
    }
